...
for index, row in rates_frame.iterrows():
    ...
    elif position == 1 and (row['close'] - entry_price) <= STOPLOSS:
        position = 0
        exit_index = rates_frame.index.get_loc(index) + 1  # Próximo índice após o sinal de venda
        exit_price = rates_frame.loc[rates_frame.index[exit_index], 'open']
        exit_date = rates_frame.index[exit_index]
        strategy_return = (STOPLOSS*1.1)
        trade_records.append({
            'Entry_Date': entry_date,
            'Exit_Date': exit_date,  # Deve ser igual a exit_index
            'Profit': strategy_return
...
print('Retorno total da estratégia: {:.2f} Pontos'.format(total_return))
...
print(f"Média de ganho por operação: {average_gain:.2f} Pontos")
print(f"Média de perda por operação: {average_loss:.2f} Pontos")
print(f"Retorno médio por operação: {average_profit_per_trade:.2f} Pontos")
...
print(trade_records_df.describe())
